...
function getAreaPath(data, width = 240, height = 64) {
...
// --- Ledger row ---
function LedgerRow({ stat, ratePercent }) {
  const s = SEMANTIC_STYLES[stat.semantic];
  return (
      ...
      if (response && response.success) {
        ...
        setStats({
          hero: [
            {
              title: 'Total Revenue',
              value: `$${(overview?.totalRevenue || 0).toLocaleString(undefined, {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              })}`,
              meta: 'This period',
              semantic: 'hero',
              trendData: trends?.revenueTrend || [],
            },
            {
              ...
              meta: 'Per order',
              semantic: 'neutral',
              trendData: trends?.aovTrend || [],
            },
          ],
          statuses: [
            { title: 'Total Orders', value: orderCounts?.total || 0, meta: 'Filtered', semantic: 'neutral' },
            { title: 'Pending', value: orderCounts?.pending || 0, meta: 'Awaiting', semantic: 'warning' },
            { title: 'Shipped', value: orderCounts?.shipped || 0, meta: 'In transit', semantic: 'neutral' },
            { title: 'Needs Action', value: overview?.actionableOrdersCount || 0, meta: 'Action req.', semantic: 'warning' },
          ],
          breakdown: [
            { title: 'Confirmed', value: orderCounts?.confirmed || 0, semantic: 'neutral' },
            { title: 'Processing', value: orderCounts?.processing || 0, semantic: 'neutral' },
            { title: 'Delivered', value: orderCounts?.delivered || 0, semantic: 'positive' },
            { title: 'Cancelled', value: orderCounts?.cancelled || 0, semantic: 'danger' },
          ],
        });
      } else {
