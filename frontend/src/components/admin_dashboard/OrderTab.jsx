...
import { getAllOrdersAdmin } from '../../services/order.service.js';
...
export default function OrderTab({
  ...
  const fetchOrdersAndUsers = useCallback(async (pageNum, isLoadMore = false) => {
    try {
      if (isLoadMore) {
        setLoadingMore(true);
      } else {
      ...
      const uncachedUserIds = Array.from(
        new Set(
          rawOrders
            ...
            .filter((id) => Boolean(id) && !userCache.has(id))
        )
      );
