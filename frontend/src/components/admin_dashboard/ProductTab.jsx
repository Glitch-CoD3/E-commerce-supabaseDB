...
import { compressImage } from '../../services/compressImage.js';
...
import ProductForm from './productTapComponents/ProductForm.jsx';
import ProductTable from './productTapComponents/ProductTable.jsx';


const emptyDraft = { color: '', size: '', price: '', stock_quantity: '' };

// --- small helpers (kept here since we're not splitting out a utils file) ---

export const getVariantColor = (v) => v.colors || '';
export const getVariantSize = (v) => v.sizes || '';
...
export default function ProductTab({
  ...
  const [variantDraft, setVariantDraft] = useState(emptyDraft);
  const [savingVariantId, setSavingVariantId] = useState(null);

  const [addingVariantFor, setAddingVariantFor] = useState(null);
  const [newVariantDraft, setNewVariantDraft] = useState(emptyDraft);
  const [creatingVariant, setCreatingVariant] = useState(false);

  const [busyImageKey, setBusyImageKey] = useState(null);
  ...
  const fetchProductsPage = async (page, mode) => {
    setIsFetchingData(true);
    try {
  ...
  const saveVariantEdit = async (productId, variantId) => {
  ...
  const createVariant = async (productId) => {
    ...
    try {
      ...
      const data = res.data || {};
      const created = data.created_varient;
  ...
  const addVariantImage = async (productId, variantId, file) => {
    ...
    try {
      ...
      const data = res.data || {};
      const newImage = data.data || data.image || data;
  ...
  const deleteVariantImage = async (productId, variantId, imageId) => {
